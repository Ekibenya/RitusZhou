...
> 使用本游戏即表示你同意 **[《使用条款与免责声明》（TERMS.md）](TERMS.md)** 全部内容。以下为要点摘录，完整具法律效力的条款以 TERMS.md 为准。

- **本项目是纯前端静态网页，无服务器端业务程序、无数据库。** 你的一切**游戏数据**——存档、聊天记录、太史长卷记忆、世界书、预设、正则、脚本、插画、AI 接口密钥与全部设置——**只保存在你这台设备的浏览器本地存储（localStorage / IndexedDB）里，不会上传本站，本项目与作者亦无从收集或查看。** 设置中的「清除本机数据」可随时一键抹除。
- **唯一离开你浏览器的游戏数据，是你自己主动配置的 AI / 绘图 / 语音接口的请求**——由你的浏览器直接发往你所选定的服务商，本项目与作者看不到、也接触不到其内容。密钥同样仅存本机、仅发往你所填的地址。
- **访问统计**：在线站点可能启用隐私友好型的匿名聚合网页统计（如 Vercel Web Analytics）以了解访问概况——**不使用 Cookie、不识别个人身份、不跨站追踪**，仅采集匿名聚合访问数据，且**绝不接触你的存档、聊天、记忆、密钥等任何游戏数据**；离线下载版不含任何此类统计。
- **用户在本游戏中输入、生成、保存、导出、分享的一切内容，以及一切游玩行为与使用方式，均由使用者独立完成并自负全部法律与道义责任，完全与本项目及作者无关。** 作者不产生、不拥有、不控制、不存储、不审阅、不背书任何用户生成内容。使用者须遵守其所在司法管辖区的全部适用法律法规，以及所接入第三方服务商的使用条款。
- 本游戏按 **「现状」（AS IS）** 提供，不附带任何明示或默示担保；在适用法律允许的最大范围内，作者对因使用本游戏、任何用户内容或所接入第三方服务而产生的任何损害不承担责任。详见 [TERMS.md](TERMS.md)。
