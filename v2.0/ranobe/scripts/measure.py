...
【周纪版】mongol/tools/ranobe/scripts/measure.py の周纪カード特化版。
差分は4点だけ（他は一字も変えていない）：
  C7  世界説明の検出語を「馬娘」→「不老の周天子」に差し替え
  Q1  明清・後世要素の禁令（このカードの鉄則。開局のみ発火）
  C24 見下し語から「奴才」を外した（明清語なのでQ1と衝突するため）
  JIANGHU「老子」に例外パターン（哲学者の老子＝老聃を叩かないため）
...
# 周纪版：哲学者「老子（老聃）」はこのカードの主要人物なので、
# 一人称の粋がり「老子（おれさま）」だけを叩く。「老子曰」「老子说」等は除外。
JIANGHU_EXEMPT = {
    '老子': re.compile(r'老子(?![曰说道])'),
}
...
# 周纪版 C7：この世界の前提＝「周の天子は年を取らない」。
# どの開局から入ってもこの一言だけで世界が分かること。
# 「不老側の語」と「天子側の語」が 50 字以内・改行を跨がずに同居していれば合格。
# （原版は一文内の前後関係を固定していたが、语序が自由な中国語では取りこぼすので
#   周纪版は距離判定にした。判定の厳しさは同等。）
ZW_IMMORTAL = re.compile(r'不会老|不老|不死|永生|长生|杀不死|一夜(?:就)?长好|老不了|'
                         r'活了[^。]{0,8}年|[两三四五六七八九十百千]百年|千年|'
                         r'一直是这个岁数|停在')
ZW_SOVEREIGN = re.compile(r'天子|周王|王上|予一人|姬瑶|这张脸|九鼎|我')


def zhou_world(raw):
    """不老の語と天子の語が近接して同居する箇所があるか。"""
    a = [m.start() for m in ZW_IMMORTAL.finditer(raw)]
    b = [m.start() for m in ZW_SOVEREIGN.finditer(raw)]
    for i in a:
        for j in b:
            if abs(i - j) <= 50 and '\n' not in raw[min(i, j):max(i, j)]:
                return True
    return False


# 周纪版 Q1：このカードの鉄則＝明清・後世の名物を先秦〜唐の本文に混ぜない。
# 部分一致の誤検出を避けるため、語は長めのものだけ採る。開局のみ発火。
LATE_ERA = ['丫鬟', '太监', '银子', '银两', '银票', '铜板', '圣旨', '奉天承运', '娘娘',
            '万岁爷', '奴才', '皇上', '太师椅', '青楼', '妓院', '衙门', '一炷香',
            '一盏茶', '女诫', '三从四德', '贞节', '缠足', '浸猪笼', '八股', '辣椒',
            '花生', '玉米', '土豆', '西瓜', '炒菜', '白酒', '烧酒', '石狮子', '牌坊']
...
FIXHINT = {
    ...
    'Q1': '明清・後世の名物が混じっている。丫鬟→侍女／太监→寺人・宦者／银子→贝・布币・刀币・黄金／'
           '圣旨→王命・策命／一炷香→食时・晡时。当該時代に実在する物へ',
...
def score(path):
    ...
    # 7) 世界そのものの説明：ウマ娘とは何かを毎回書く。
    #    どの開局から入っても、その一篇だけで世界が分かること。
    if is_opening and not zhou_world(raw):
        struct.append(('C7', '-', '「不老の周天子」という世界の前提説明が無い（毎篇に必要）'))
    ...
    # 24) 見下しが地の語彙になっているか。
    LOOKDOWN = r'这帮|那帮|这些东西|这东西|牲口|不算人|贱民|贱人|杂碎|一群|狗一样|畜生一样'
    ...
    # 26) 周纪版 Q1：明清・後世要素。このカードでは時代考証が鉄則なので文体より上位。
    if is_opening:
        for w in LATE_ERA:
            if w in raw:
                struct.append(('Q1', w, '明清・後世の名物。先秦〜唐に存在しない語'))
